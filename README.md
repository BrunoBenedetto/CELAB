...
| **Dashboard** | Hero + 4 indicadores de situação, 4 gráficos, atalhos e Exportar Geral |
| **Estoque Laboratório** | Tabela do inventário, cadastro direto, edição, exclusão, filtros e ordenação |
| **Entrada de Equipamentos** | Formulário de chegada + últimas 20 entradas |
| **Saída de Equipamentos** | Formulário de envio com busca por tombo + últimas 20 saídas |
| **Relatórios e Filtros** | Histórico completo com 10 filtros e exportação da fatia filtrada |

Atalhos: `Alt+1` … `Alt+5` alternam entre as abas. O botão 🌙/☀️ troca claro/escuro.
...
**Status e o que conta como estoque**

| Status | No laboratório? | Cor |
|---|---|---|
| Estoque | sim | verde (good) |
| Manutenção | sim | âmbar (warning) |
| Defeito | sim | vermelho (critical) |
| Leilão | sim | laranja (serious) |
| Disponibilizado | **não** | cinza (neutro) |

O indicador "Equipamentos no laboratório" e o gráfico de composição contam só os quatro
primeiros. "Disponibilizado" aparece no rodapé do indicador principal e no filtro
*Localização → Disponibilizados* da aba Estoque.

**Histórico** — toda operação grava uma movimentação: `ENTRADA`, `SAIDA`, `CADASTRO`,
`AJUSTE` (com status anterior e novo) e `EXCLUSAO`. Nada se altera sem deixar rastro.
...
├── js/
│   ├── config.js           listas estáticas: 14 equipamentos, 45 modelos, 32 prédios,
│   │                       status, TTR, perfis e o mapa equipamento → modelos
│   ├── ui.js               utilitários, 30 ícones SVG, toast, modal, campos, validação
...
└── tests/
    ├── autoteste.html      48 testes automatizados (abra no navegador)
...
Tudo em [js/config.js](js/config.js): `EQUIPAMENTOS`, `MODELOS`, `PREDIOS`, `STATUS`, `TTR`
e `MODELOS_POR_EQUIPAMENTO` (o mapa que filtra o select de modelo).

O mapa é uma *sugestão*: o select mostra "Modelos de <categoria>" e, abaixo, "Outros
modelos" com todo o resto — nenhum modelo fica inacessível se a classificação divergir do
inventário real.
...
Abra [tests/autoteste.html](tests/autoteste.html) no navegador. São 48 verificações
cobrindo listas estáticas, utilitários, todas as regras de entrada/saída, colisão de
tombo, histórico, backup/restauração, autenticação, permissões, paleta nos dois temas,
geração real de XLSX (com ida-e-volta) e de PDF, e a montagem das cinco páginas.

Última execução: **48 aprovados, 0 falhas** (Chrome headless, perfil limpo) — incluindo
um XLSX de 28 KB relido e conferido e um PDF de 12 páginas gerado em memória.
...
`estoque`, `entrada`, `saida`, `relatorios`; `?login=1` abre a tela de login).
...
em JSON. Zero infraestrutura, zero custo.
...
create table equipamentos (
  ...
  status text not null check (status in
    ('Estoque','Manutenção','Defeito','Leilão','Disponibilizado')),
  chamado text,
  servico_solicitado text,
  ttr text check (ttr in ('Realizado','Pendente')),
  data_entrada date,
  predio_origem text,
  setor_origem text,
  data_saida date,
...
create table movimentacoes (
  ...
  tipo text not null check (tipo in
    ('ENTRADA','SAIDA','CADASTRO','AJUSTE','EXCLUSAO')),
  data date not null,
  status_anterior text,
  status_resultante text,
  predio text, setor text, chamado text, ttr text,
...
alter publication supabase_realtime add table equipamentos, movimentacoes;
```
